    ...
    return{
        ...
        postService(req,res){
            try {
                ...
                Info.findOne({ MACPORT: data.MACPORT},function (err, mp){
                    if(err) throw err
                    if(!mp){
                        const ip = req.headers['x-forwarded-for'] ||  req.connection.remoteAddress;
                        const connectDate = Date.connectDate()
                        ...
                        const infoData = { SERVERNAME:ServerName, MAC:data.MAC, IP:data.IP, PORT:data.PORT, MACPORT:socketData}
                        const logDb = { log: `API::POST::${connectDate}::${ServerName}::${infoData.MAC}::${ip}::${logOpenDay}::/SocketServerCreate` }

                        const turnData = {ip:ip, SERVERNAME:ServerName, MAC:data.MAC, IP:data.IP, PORT:data.PORT, MACPORT:socketData }
                        ...
                        res.status(200).send('Socket Server Creation Completed',ServerName);
                    }else{
                        return res.status(400).send('이미 사용중인 MACPORT 입니다.')
                    }
                })
